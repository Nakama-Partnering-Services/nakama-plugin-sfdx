...
Verifies that specified apex classes have a coverage above a certain treshold. Useful to improve your CICD processes with enhanced feedback about your apex test coverage. This command is based on the result files generated by other commands such as sfdx force:source:deploy.

```
$ sfdx nps:coverage:verify -h
...
FLAGS
  -c, --classes=<value>                                                             (required) comma separated list of apex classes to verify
  -p, --path=<value>                                                                (required) project relative path to the json file
                                                                                    containing the test execution results
  -r, --requiredcoverage=<value>                                                    [default: 75] number of the percentage of coverage
                                                                                    treshold to reach
...
_See code: [src/commands/nps/coverage/verify.ts](https://github.com/Nakama-Partnering-Services/nakama-plugin-sfdx/blob/main/src/commands/nps/coverage/verify.ts)_
